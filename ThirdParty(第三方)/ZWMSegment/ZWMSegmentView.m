...
- (void)segmentBasicSetting {
    ...
    _font = [UIFont boldSystemFontOfSize:18];//[UIFont systemFontOfSize:16];
...
//            _indicateView.frame = CGRectMake(_buttonSpace-50, _segmentHeight - _indicateHeight, titleSize.width+50+50, _indicateHeight);
             _indicateView.frame = CGRectMake(button.centerX-titleSize.width/2, _segmentHeight - _indicateHeight, titleSize.width, _indicateHeight);
            [_contentView addSubview:_indicateView];
        }
        if (i<_titles.count - 1) {
            UIView * shuView = [[UIView alloc]initWithFrame:CGRectMake(button.right, 12, 1, 20)];
            shuView.backgroundColor = WhiteBackColor;
//            [_contentView addSubview:shuView];
        }
...
//             _indicateView.frame = CGRectMake(_selectedButton.centerX-16, CGRectGetMinY(_indicateView.frame)-3, 32, _indicateHeight);
        }
    } completion:nil];
}
